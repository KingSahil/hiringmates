...
export async function GET(request: Request) {
  ...
  if (actor) {
    ...
    // Fetch polished profiles generated by the RAG system
    ...
    const profileByUserId = new Map(
      (candidateProfiles ?? []).map((cp: any) => [cp.user_id, cp.payload]),
    )

    // Fallback: check completed sessions if candidate_profiles does not have entry
    const missingStudentIds = studentIds.filter((id) => !profileByUserId.has(id))
    if (missingStudentIds.length > 0) {
      ...
      for (const s of completedSessions ?? []) {
        if (!profileByUserId.has(s.user_id) && s.payload?.enhanced) {
          profileByUserId.set(s.user_id, s.payload.enhanced)
        }
      }
    }
