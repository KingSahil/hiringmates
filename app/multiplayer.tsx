...
export function MultiplayerGame({ nav, room }: { nav: Nav; room: Room }) {
  const supabase = getSupabaseBrowserClient(); const [code, setCode] = useState("const tasks = []\n\nfunction addTask(title) {\n  tasks.push({ title, done: false })\n}"); const [messages, setMessages] = useState<{ body: string; user_id: string }[]>([]); const [chat, setChat] = useState(''); const [online, setOnline] = useState(1); const [channel, setChannel] = useState<ReturnType<typeof supabase.channel> | null>(null)
  useEffect(() => {
    let disposed = false
    ...
    realtimeChannel.on('broadcast', { event: 'code-change' }, ({ payload }) => { if (!disposed) setCode(payload.code) }).on('broadcast', { event: 'chat' }, ({ payload }) => { if (!disposed) setMessages(current => [...current, payload]) }).on('presence', { event: 'sync' }, () => { if (!disposed) setOnline(Object.keys(realtimeChannel.presenceState()).length) }).subscribe(async status => { if (status === 'SUBSCRIBED' && !disposed) await realtimeChannel.track({ online_at: new Date().toISOString() }) })
    load()
