...
export function MultiplayerGame({ nav, room }: { nav: Nav; room: Room }) {
  ...
  useEffect(() => {
    ...
    realtimeChannel.on('broadcast', { event: 'code-change' }, ({ payload }) => { if (!disposed) setCode(payload.code) }).on('broadcast', { event: 'cursor-move' }, ({ payload }) => { if (!disposed) setRemoteCursors(current => ({ ...current, [payload.userId]: { line: payload.line, column: payload.column } })) }).on('broadcast', { event: 'chat' }, ({ payload }) => { if (!disposed) setMessages(current => [...current, payload]) }).on('presence', { event: 'sync' }, () => { if (!disposed) setOnline(Object.keys(realtimeChannel.presenceState()).length) }).subscribe(async status => { if (status === 'SUBSCRIBED' && !disposed) await realtimeChannel.track({ online_at: new Date().toISOString() }) })
    load()
