...
export function AuthPage({ nav }: { nav: Nav }) {
  ...
  const verifyEmailCode = async () => {
    const normalizedEmail = email.trim().toLowerCase()
    if (!normalizedEmail || otp.trim().length < 6) return setMessage('Enter the six-digit code from your email.')
    setBusy(true)
    const { data, error } = await getSupabaseBrowserClient().auth.verifyOtp({ email: normalizedEmail, token: otp.trim(), type: 'email' })
    if (error || !data.session) setMessage('That code is invalid or expired. Request a new code.')
    else { setMessage('Signed in.'); nav('home') }
    setBusy(false)
  }
...
export function RealLobby({ nav, room, onStart }: { nav: Nav; room: Room; onStart: () => void }) {
  ...
  useEffect(() => {
    let disposed = false
    const channel = supabase.channel(`room:${room.id}:${crypto.randomUUID()}`)
    const join = async () => {
      const { data: { user } } = await supabase.auth.getUser()
      if (!disposed && user) await supabase.from('room_members').upsert({ room_id: room.id, user_id: user.id })
      if (!disposed) load()
    }
    channel.on('postgres_changes', { event: '*', schema: 'public', table: 'room_members', filter: `room_id=eq.${room.id}` }, load).subscribe()
    join()
    return () => { disposed = true; channel.unsubscribe(); void supabase.removeChannel(channel) }
  }, [room.id])
...
export function MultiplayerGame({ nav, room }: { nav: Nav; room: Room }) {
  const supabase = getSupabaseBrowserClient(); const [playerId] = useState(() => crypto.randomUUID()); const [code, setCode] = useState("const tasks = []\n\nfunction addTask(title) {\n  tasks.push({ title, done: false })\n}"); const [messages, setMessages] = useState<{ body: string; user_id: string }[]>([]); const [chat, setChat] = useState(''); const [online, setOnline] = useState(1); const [remoteCursors, setRemoteCursors] = useState<Record<string, { line: number; column: number }>>({}); const [channel, setChannel] = useState<ReturnType<typeof supabase.channel> | null>(null)
  useEffect(() => {
    let disposed = false
    const realtimeChannel = supabase.channel(`game:${room.id}:${crypto.randomUUID()}`, { config: { presence: { key: crypto.randomUUID() }, broadcast: { self: false } } })
    setChannel(realtimeChannel)
    const load = async () => { const { data } = await supabase.from('messages').select('body,user_id').eq('room_id', room.id).order('created_at'); if (!disposed) setMessages(data ?? []) }
    realtimeChannel.on('broadcast', { event: 'code-change' }, ({ payload }: any) => { if (!disposed) setCode(payload.code) }).on('broadcast', { event: 'cursor-move' }, ({ payload }: any) => { if (!disposed) setRemoteCursors(current => ({ ...current, [payload.userId]: { line: payload.line, column: payload.column } })) }).on('broadcast', { event: 'chat' }, ({ payload }: any) => { if (!disposed) setMessages(current => [...current, payload]) }).on('presence', { event: 'sync' }, () => { if (!disposed) setOnline(Object.keys(realtimeChannel.presenceState()).length) }).subscribe(async (status: string) => { if (status === 'SUBSCRIBED' && !disposed) await realtimeChannel.track({ online_at: new Date().toISOString() }) })
    load()
    return () => { disposed = true; realtimeChannel.unsubscribe(); void supabase.removeChannel(realtimeChannel) }
  }, [room.id])
  const updateCode = (value = '') => { setCode(value); channel?.send({ type: 'broadcast', event: 'code-change', payload: { code: value } }) }
  const broadcastCursor = (position: { lineNumber: number; column: number }) => { channel?.send({ type: 'broadcast', event: 'cursor-move', payload: { userId: playerId, line: position.lineNumber, column: position.column } }) }
